...
aug = naw.RandomWordAug(action="crop", aug_min=1, aug_max=5, aug_p=0.1)
tt = TweetTokenizer()


def random_crop(data_frame):
    augmented_dataset = []
    mal_cnt = 0
    ...
    for article in tqdm(data_frame.itertuples()):
        ...
        try:
            augmented_text = aug.augment(body_texts)[0]
        except ValueError as e:
            mal_cnt += 1
            augmented_text = body_texts

        # print(augmented_text)
        # print(len(augmented_text))   # 1

        augmented_dataset.append([article.full_text, augmented_text, article.image_id,
                                  article.filename, article.falsified, article.topic, article.exists])
    
    augmented_frame = pd.DataFrame(augmented_dataset, columns=["full_text", "full_text_random_crop", "image_id",
                                                               "filename", "falsified", "topic", "exists"])
    print(f"Number of malformed lines: {mal_cnt}")
...
if __name__ == "__main__":
    ...
    augmented_df = random_crop(df)
    augmented_df.to_feather('./raw_data/toy_completed_exist_random_crop.feather')
