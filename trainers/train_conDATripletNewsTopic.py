"""
(venv_py38) python -m trainers.train_conDATripletNewsTopic --batch_size 256 --max_epochs 20 --target_domain bbc,guardian --base_model blip-2 --loss_type simclr
"""
import logging
import os
import subprocess
...
from dataset.newsCLIPpingsDatasetConDATripletTopic import get_dataloader
...
def validate(model: nn.Module, device: str, loader: DataLoader, votes=1, desc='Validation'):
    ...
    topic_validation_accuracy = dict()
    topic_epoch_size = dict()
    topics = {
    ...
    for topic in topics.keys():
        topic_validation_accuracy[topic] = 0
        topic_epoch_size[topic] = 0
    ...
    with tqdm(records, desc=desc) as loop, torch.no_grad():
        for example in loop:
            ...
            topic_ind = dict()
            for topic in topics.keys():
                topic_ind[topic] = np.array([i for i, d_label in enumerate(domain_labels) if d_label==topic])
            ...
            for topic in topics.keys():
                topic_validation_accuracy[topic] += accuracy_sum(logits[topic_ind[topic]], labels[topic_ind[topic]])
            
            for topic in topics.keys():
                topic_epoch_size[topic] += topic_ind[topic].shape[0]


            loop.set_postfix(loss=loss.item(), acc=validation_accuracy / validation_epoch_size, 
                             law_crime_acc=topic_validation_accuracy["law_crime"] / topic_epoch_size["law_crime"], media_acc=topic_validation_accuracy["media"] / topic_epoch_size["media"],
                             education_acc=topic_validation_accuracy["education"] / topic_epoch_size["education"], books_acc=topic_validation_accuracy["books"] / topic_epoch_size["books"],
                             society_acc=topic_validation_accuracy["society"] / topic_epoch_size["society"],edinburgh_acc=topic_validation_accuracy["edinburgh"] / topic_epoch_size["edinburgh"],
                             leeds_acc=topic_validation_accuracy["leeds"] / (topic_epoch_size["leeds"] + 0.01),stage_acc=topic_validation_accuracy["stage"] / (topic_epoch_size["stage"]+0.01))
...
def run(cfg, device):
    ...
    tgt_excluded_topic = ["law_crime",# "law",
                  "media",
                  "education",
                  "books",
                  "society",
