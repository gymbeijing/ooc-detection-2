    ...
    for epoch in range(EPOCHS):
        ...
        for i, batch in tqdm(enumerate(train_iterator, 0), desc='iterations'):
            ...
            if epoch < cfg.args.pre_train_epochs:
                y_preds = net.first_stage(inputs)
                loss = cross_entropy_loss(y_preds, labels)

                # Back-propagate and update the parameters
                loss.backward()
                main_optimizer.step()   # stage 1
            else:
                y_preds = net.second_stage(inputs)
                loss = cross_entropy_loss(y_preds, labels)
        ...
        test_pred, test_true = test(net, val_iterator, cross_entropy_loss, device)
