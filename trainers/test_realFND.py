...
if __name__ == '__main__':
    ...
    val_data = TwitterCOMMsDataset(feather_path='./raw_data/val_completed_exist.feather',
                                   ...
                                   few_shot_topic=['military', 'climate']
                                   )
    ...
    fake_news_classifier.load_state_dict(torch.load(os.path.join('./', 'real_fnd_output', 'fake_news_classifier_covid.ckpt')))

    policy_network = Policy(768 * 2)
    policy_network.cuda()
    policy_network.eval()
    policy_network.load_state_dict(torch.load(os.path.join('./', 'real_fnd_output', 'policy_network.ckpt')))
