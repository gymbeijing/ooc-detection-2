...
class NewsCLIPpingsDatasetConDATripletTopic(Dataset):
    def __init__(self, img_dir, original_multimodal_embeds_path, positive_multimodal_embeds_path, 
                 negative_multimodal_embeds_path, label_path, topic_path, target_domain=None, phase="test"):
        """
        Args:
            ...
            topic_path (string): path to the file that stores the topics
        ...
        self.domain_labels = load_json(topic_path)["topic"]
        ...
        # print(f"phase: {self.phase}, excluded domain: {self.target_domain}")
            row_excluded = [i for i, x in enumerate(self.domain_labels) if x in target_domain or x not in topics]
...
def get_dataset(root_dir, data_dir, img_dir, split, phase, target_domain):
    ...
    dataset = NewsCLIPpingsDatasetConDATripletTopic(img_dir, original_multimodal_embeds_path, positive_multimodal_embeds_path, negative_multimodal_embeds_path, label_path, topic_path, target_domain, phase)
    return dataset
